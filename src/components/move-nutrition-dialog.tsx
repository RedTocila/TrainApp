...
interface MoveNutritionDialogProps {
  open: boolean;
  onClose: () => void;
  planId: string;
  planTitle: string;
...
export function MoveNutritionDialog({
  open,
  onClose,
  planId,
  planTitle,
  ...
  const current = resolveNutritionFolderId(currentFolderId);
  const destinations = folders.filter((f) => f.id !== current);

  const handleMove = (targetFolderId: string) => {
    setError(null);
    startTransition(async () => {
      const result = await moveNutritionPlanToFolder(planId, targetFolderId);
      if (result.error) {
...
export function MoveNutritionButton({
  planId,
  planTitle,
  currentFolderId,
  folders,
