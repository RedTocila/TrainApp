...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, ...props }, ref) => (
    <input
      type={type}
      className={cn(
        "h-[var(--control-height)] w-full min-w-0 max-w-full rounded-xl border border-border/80 bg-background/70 px-3 py-2 text-sm text-foreground shadow-sm backdrop-blur-sm placeholder:text-muted-foreground",
