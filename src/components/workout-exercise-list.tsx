...
export function WorkoutExerciseList({
  ...
  const previousLabel = (name: string) =>
    ...
    return (
      <ul className={cn("divide-y divide-border/60", className)}>
        {exercises.map((exercise) => {
          const historyLabel = previousLabel(exercise.name);
          return (
            <li
              key={exercise.id}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs font-medium leading-relaxed text-primary/90">
                  {historyLabel}
                </p>
              </div>
            </li>
      ...
      <ul className="space-y-2">
        {exercises.map((exercise, index) => {
          ...
          return (
            <li
              key={exercise.id}
              className={cn(dashboard.listRow, "items-start rounded-3xl py-3")}
            >
              <ExerciseGifThumbnail
                name={exercise.name}
                imageUrl={exercise.image_url}
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-2 text-xs font-medium leading-relaxed text-primary/90">
                  {historyLabel}
                </p>
              </div>
            </li>
